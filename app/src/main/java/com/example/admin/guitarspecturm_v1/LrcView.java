...
//        drawLrc1(canvas);
    ...
    private void drawLrc2(Canvas canvas, int currentMillis) {
        if (mode == 0) {
            ...
            long start = lrcBean.getBeginDt();
            long end = lrcBean.getEndDt();
    ...
    private void getCurrentPosition() {
        try {
            int currentMillis = player.getCurrentPosition();
            if (currentMillis < list.get(0).getBeginDt()) {
                currentPosition = 0;
                return;
            }
            if (currentMillis > list.get(list.size() - 1).getBeginDt()) {
                currentPosition = list.size() - 1;
                return;
            }
            for (int i = 0; i < list.size(); i++) {
                if (currentMillis >= list.get(i).getBeginDt() && currentMillis < list.get(i).getEndDt()) {
                    currentPosition = i;
                    return;
                }
            }
